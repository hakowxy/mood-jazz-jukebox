...
# --- 1. Music Data (UPDATED with your 6 links) ---
# NOTE: The links are assigned to genres based on common usage and video titles.
MUSIC_DATA = {
    # 适合高能量/兴奋 (8-10分) 或通勤
    "City Pop": [
...
def recommend_music(mood_score, location):
    ...
    using only City Pop, Jazz HipHop, and Ambient Jazz.
    """

    # A. Mood-based primary determination
    if mood_score >= 8:
        # High Energy -> City Pop (Upbeat/Urban)
        genre = "City Pop"
    elif mood_score >= 5:
        # Mid Energy -> Jazz HipHop (Chill/Focus)
        genre = "Jazz HipHop"
    else: # 1-4
        # Low Energy -> Ambient Jazz (Relaxation/Calm)
        genre = "Ambient Jazz"

    # B. Location-based refinement
    if location == "Commuting":
        # Commuting often needs rhythm, so we favor City Pop/Jazz HipHop
        if mood_score < 5:
             # Even if tired, need something with a beat for travel
            genre = "Jazz HipHop"
        else:
            genre = "City Pop"
    elif location == "Park/Outdoor":
        # Outdoor calls for brighter, uplifting sounds
        if mood_score < 6:
            genre = "Jazz HipHop"
        else:
            genre = "City Pop"
    elif location == "Home":
        # Home calls for deep relaxation
        if mood_score < 6:
            genre = "Ambient Jazz"
        else:
            genre = "Jazz HipHop" # Chill but not sleepy

    return genre

# --- 3. Streamlit UI (Pure Python for HTML/CSS) ---
